...
app.post("/inventory", async (req, res) => {
  try {
    ...
    console.error("Error in /inventory:", err); // *** CHANGED - Added console.error ***
...
app.post("/financial-report", async (req, res) => {
  try {
    ...
    const totalsPromises = VAMCIds.map((id) =>
      pool.request().input("VAMC_Id", sql.Int, id).execute("CPA_Total"),
    );
...
app.post("/issues-by-station", async (req, res) => {
  try {
    ...
    console.error("Error in /issues-by-station:", err); // *** CHANGED - Fixed endpoint name in log ***
    res.status(500).json({ error: "Database query failed" }); // *** CHANGED - Generic error message ***
  }
});
